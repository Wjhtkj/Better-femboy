...
// 服务端兜底阈值：50 道题在 8 秒内答完基本不可能。
// 客户端已有一套反作弊，这里再兜一层，防止构造请求直接绕过前端。
...
export async function onRequestPost({ request, env }) {
  ...
  const rawPercent = optionalNumber(body.scorePercent) ?? (score / scoreMax) * 100;
  const scorePercent = Math.max(0, Math.min(100, rawPercent));

  const durationSeconds = optionalNumber(body.durationSeconds);
  const isSpam = Boolean(body.isSpam)
    || (durationSeconds != null && durationSeconds > 0 && durationSeconds < MIN_PLAUSIBLE_SECONDS);
